...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^sokp_") ; then
       SOKP_BUILD=$(echo -n $1 | sed -e 's/^sokp_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $SOKP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       SOKP_BUILD=
    fi
    export SOKP_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the SOKP github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var SOKP_VERSION)
        ZIPFILE=sokp-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.sokp.device=$SOKP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $SOKP_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SOKP_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/sokp/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
